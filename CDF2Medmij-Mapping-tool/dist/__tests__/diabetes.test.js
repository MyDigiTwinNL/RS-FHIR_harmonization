...
const Diabetes_1 = require("../lifelines/Diabetes");
...
test('diabetes clinical status, when reported positive in 1A, diabetes type 2', () => {
    ...
    expect(Diabetes_1.diabetes.clinicalStatus()?.display).toBe("Active");
    expect(Diabetes_1.diabetes.isPresent()).toBe(true);
    expect(Diabetes_1.diabetes.code().display).toBe("Diabetes mellitus type 2 (disorder)");
    expect(Diabetes_1.diabetes.onsetDateTime()).toBe("1982");
});
test('diabetes clinical status, when reported positive in a follow-up, diabetes type 1', () => {
    ...
    expect(Diabetes_1.diabetes.clinicalStatus()?.display).toBe("Active");
    expect(Diabetes_1.diabetes.isPresent()).toBe(true);
    expect(Diabetes_1.diabetes.code().display).toBe("Diabetes mellitus type 1 (disorder)");
    expect(Diabetes_1.diabetes.onsetDateTime()).toBe("2002-05");
});
test('assertion test: diabetes clinical status, when reported positive in a follow-up, but no diabetes type is defined', () => {
    ...
    expect(Diabetes_1.diabetes.code().display).toBe("Diabetes mellitus (disorder)");
    expect(Diabetes_1.diabetes.onsetDateTime()).toBe("2002-05");
});
test('diabetes clinical status, when reported positive in a follow-up after multiple skipped assessments, diabetes type 1', () => {
    ...
    expect(Diabetes_1.diabetes.isPresent()).toBe(true);
    expect(Diabetes_1.diabetes.code().display).toBe("Diabetes mellitus type 1 (disorder)");
    expect(Diabetes_1.diabetes.onsetDateTime()).toBe("2003-05");
});
test('diabetes clinical status (T1D) reported on a follow up, with missing date on the assessment where was reported', () => {
    ...
    expect(Diabetes_1.diabetes.isPresent()).toBe(true);
    expect(Diabetes_1.diabetes.code().display).toBe("Diabetes mellitus type 1 (disorder)");
    expect(Diabetes_1.diabetes.onsetDateTime()).toBe(undefined);
});
...
test('diabetes clinical status, when never reported', () => {
    ...
    inputSingleton_1.InputSingleton.getInstance().setInput(input);
    expect(Diabetes_1.diabetes.clinicalStatus()).toStrictEqual(undefined);
    expect(Diabetes_1.diabetes.isPresent()).toBe(false);
});
/**
...
//# sourceMappingURL=diabetes.test.js.map