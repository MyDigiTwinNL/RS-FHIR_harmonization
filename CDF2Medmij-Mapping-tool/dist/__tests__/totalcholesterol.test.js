...
const totalCholesterolFunctions = __importStar(require("../lifelines/TotalCholesterol"));
...
test('Cholesterol reports, above reference range', () => {
    ...
    expect(results[0].isTotalCholAboveReferenceRange).toBe(true);
    expect(results[0].resultFlags).toBe(snomedCodeLists_1.testResultFlagsSNOMEDCodelist.above_reference_range);
    expect(results[1].isTotalCholAboveReferenceRange).toBe(true);
...
test('Cholesterol reports, mix of normal and above reference ranges', () => {
    ...
    const results = totalCholesterolFunctions.results();
    expect(results.length).toBe(2);
    expect(results[0].isTotalCholAboveReferenceRange).toBe(false);
...
test('Cholesterol reports, within normal levels', () => {
    ...
    expect(results[0].isTotalCholAboveReferenceRange).toBe(false);
    expect(results[0].resultFlags).toStrictEqual(undefined);
    expect(results[1].isTotalCholAboveReferenceRange).toBe(false);
...
//# sourceMappingURL=totalcholesterol.test.js.map