...
const HbA1c_1 = require("../lifelines/HbA1c");
test('HBA1C above upper limit', () => {
    ...
    const results = HbA1c_1.hbA1c.results();
    expect(results.length).toBe(2);
    expect(results[0].testResult).toBe(43);
    expect(results[0].resultFlags?.display).toBe("Above reference range");
    expect(results[1].testResult).toBe(44);
...
test('HBA1C below upper limit', () => {
    ...
    const results = HbA1c_1.hbA1c.results();
    expect(results.length).toBe(2);
    expect(results[0].testResult).toBe(12);
    expect(results[0].resultFlags).toBe(undefined);
    expect(results[1].testResult).toBe(20);
    expect(results[1].resultFlags).toBe(undefined);
});
//# sourceMappingURL=hba1c.test.js.map