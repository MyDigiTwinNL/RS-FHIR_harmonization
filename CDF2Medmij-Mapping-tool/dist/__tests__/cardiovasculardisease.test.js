...
const CardioVascularDisease_1 = require("../lifelines/CardioVascularDisease");
...
test('CVD , when none of the related conditions (MI, HF, Stroke) are present', () => {
    ...
    expect(CardioVascularDisease_1.cardioVascularDisease.clinicalStatus()).toStrictEqual(undefined);
});
test('CVD , when the three related conditions (MI, HF, Stroke) are present on the baseline assessment', () => {
    ...
    inputSingleton_1.InputSingleton.getInstance().setInput(input);
    expect(CardioVascularDisease_1.cardioVascularDisease.clinicalStatus()?.display).toBe("Active");
    expect(CardioVascularDisease_1.cardioVascularDisease.isPresent()).toBe(true);
    expect(CardioVascularDisease_1.cardioVascularDisease.code().display).toBe("CVD - cardiovascular disease");
    //Expected to use the 'onset' date of HF, the earliest condition to happen
    expect(CardioVascularDisease_1.cardioVascularDisease.onsetDateTime()).toBe("1977-01");
});
test('CVD , when only one of the conditions (MI, HF, Stroke) is present on the baseline assessment', () => {
    ...
    expect(CardioVascularDisease_1.cardioVascularDisease.code().display).toBe("CVD - cardiovascular disease");
    //Expected to use the 'onset' date of HF, the earliest condition to happen
    expect(CardioVascularDisease_1.cardioVascularDisease.onsetDateTime()).toBe("1987-01");
});
test('CVD , when only one of the conditions (MI, HF, Stroke) is present on a follow-up assessment', () => {
    ...
    expect(CardioVascularDisease_1.cardioVascularDisease.code().display).toBe("CVD - cardiovascular disease");
    //Expected to use the 'onset' date of HF, the earliest condition to happen
    expect(CardioVascularDisease_1.cardioVascularDisease.onsetDateTime()).toBe("1999-05");
});
test('CVD , when the three related conditions are present, but none provide an onsetDate', () => {
    ...
    expect(CardioVascularDisease_1.cardioVascularDisease.code().display).toBe("CVD - cardiovascular disease");
    //Expected to use the 'onset' date of HF, the earliest condition to happen
    expect(CardioVascularDisease_1.cardioVascularDisease.onsetDateTime()).toBe(undefined);
});
/**
...
//# sourceMappingURL=cardiovasculardisease.test.js.map