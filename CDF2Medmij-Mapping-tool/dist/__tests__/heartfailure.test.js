...
const HeartFailure_1 = require("../lifelines/HeartFailure");
...
test('heartfailure, when reported positive in 1A', () => {
    ...
    expect(HeartFailure_1.heartFailure.clinicalStatus()?.display).toBe("Active");
    expect(HeartFailure_1.heartFailure.isPresent()).toBe(true);
    expect(HeartFailure_1.heartFailure.code().display).toBe("Heart failure (disorder)");
    expect(HeartFailure_1.heartFailure.onsetDateTime()).toBe("1982");
});
test('heartfailure, when reported positive in 1A, but no start_age was reported', () => {
    ...
    expect(HeartFailure_1.heartFailure.isPresent()).toBe(true);
    expect(HeartFailure_1.heartFailure.code().display).toBe("Heart failure (disorder)");
    expect(HeartFailure_1.heartFailure.onsetDateTime()).toBe(undefined);
});
test('heart failure, when reported in 2A', () => {
    ...
    expect(HeartFailure_1.heartFailure.isPresent()).toBe(true);
    expect(HeartFailure_1.heartFailure.code().display).toBe("Heart failure (disorder)");
    expect(HeartFailure_1.heartFailure.onsetDateTime()).toBe("1999-05");
});
...
test('heart failure, when reported right after baseline (1B)', () => {
    ...
    expect(HeartFailure_1.heartFailure.isPresent()).toBe(true);
    expect(HeartFailure_1.heartFailure.code().display).toBe("Heart failure (disorder)");
    expect(HeartFailure_1.heartFailure.onsetDateTime()).toBe("1993-05");
});
test('heart failure, when reported in 2A, after skipping one assessment', () => {
    ...
    expect(HeartFailure_1.heartFailure.isPresent()).toBe(true);
    expect(HeartFailure_1.heartFailure.code().display).toBe("Heart failure (disorder)");
    expect(HeartFailure_1.heartFailure.onsetDateTime()).toBe("1998-05");
});
test('heart failure, when reported in 2A, after skipping multiple assessments', () => {
    ...
    expect(HeartFailure_1.heartFailure.isPresent()).toBe(true);
    expect(HeartFailure_1.heartFailure.code().display).toBe("Heart failure (disorder)");
    expect(HeartFailure_1.heartFailure.onsetDateTime()).toBe("1997-05");
});
test('heart failure, when reported in 2A, after skipping multiple assessments', () => {
    const input = {
        "heartfailure_startage_adu_q_1": { "1a": "" },
...
test('heart failure, when no reported', () => {
    ...
    inputSingleton_1.InputSingleton.getInstance().setInput(input);
    expect(HeartFailure_1.heartFailure.clinicalStatus()).toStrictEqual(undefined);
});
...
//# sourceMappingURL=heartfailure.test.js.map